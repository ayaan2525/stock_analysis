...
company_code = ("AAPL", "GOOGL", "MSFT")				# Name and code of the companies whose data will be scraped and analysed
...
close_price.dropna(how="any", inplace=True)         # Removing all the rows w/ Nan values

# Calculating Relative Returns
rel_returns = close_price.pct_change()

rel_returns.plot(figsize=(15, 10))                 # Plotting the relative returns of all the companies
...
start = str(short_rolling.index[0]).split()[0]
...
file_name = ('AAPL_data.csv','GOOGL_data.csv','MSFT_data.csv')

for i in range(3):
    df = pd.read_csv(file_name[i])
    prices = df['Open'].tolist()                    # Prices will act as a Response vector
    dates = list(reversed(range(len(prices))))      # Dates will act as a Feature vector
    ...
    plt.scatter(dates, prices, color='blue', label= 'Actual Price')        # Plotting initial Data Points as Scatter plot
    ...
    print("\n\033[1m","Predicted Price :", predicted_price[0][0])
    print('\n\033[1m',"Regression Coefficient :", regressor.coef_[0][0])
    print('\n\033[1m',"Regression Intercept :", regressor.intercept_[0])
